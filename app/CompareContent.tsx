...
type PageEntry = { path: string; screenshot: string };
...
export function CompareContent({
  ...
  title: string;
  description: string;
  otherViewportHref: string;
  otherViewportLabel: string;
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/compare", {
        ...
        body: JSON.stringify({ url1: url1.trim(), url2: url2.trim(), viewport }),
      });
      ...
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="url1" className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">
                First URL
              </label>
              <input
                id="url1"
            ...
            <div>
              <label htmlFor="url2" className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">
                Second URL
              </label>
              <input
                id="url2"
        ...
        {result && (
          <section className="space-y-10">
            ...
            <p className="text-sm text-stone-600 dark:text-stone-400">
              {result.pages1.length} page(s) from Website 1 · {result.pages2.length} page(s) from Website 2. Same
              normalized path shown side by side.
            </p>
            <p className="text-sm text-stone-500 dark:text-stone-400">
              Only the last part of each path (after the final &quot;/&quot;) is compared, after removing .htm,
              .html, or -cc.htm.
            </p>
            <div className="space-y-6">
              {getComparisonRows(result).map((row) => (
                ...
                >
                  <div className="bg-stone-100 dark:bg-stone-800 px-4 py-2 border-b border-stone-200 dark:border-stone-700 min-w-0">
                    <span
                      className="font-mono text-sm text-stone-700 dark:text-stone-300 truncate block"
                      title={row.normalizedPath === "/" ? "/" : row.normalizedPath}
                    >
